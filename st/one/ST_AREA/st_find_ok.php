...
$sql = "select * from $datatable where traffic like '%$name%'or type like '%$name%'or locale like '%$name%'or info like '%$name%' or contacts like '%$name%'or locale like '%$name%'order by id";
...
echo '<td><div align = "center"><a onclick="javascript:if(confirm(\'确定要删除用户信息吗?\')) return true; else return false;" href="st_delete.php?id='.$row->id.'">删除</a>&nbsp;</div></td>';
echo '<td><div align = "center"><a onclick="javascript:if(confirm(\'确定要修改用户信息吗?\')) return true; else return false;" href="st_change.php?id='.$row->id.'">修改</a>&nbsp;</div></td>';
echo "</tr>";
   }
?>
</table>
