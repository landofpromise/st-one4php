...
<head>
	...
	<title>LOP ST 2015-D<?php echo $day; ?> 数据战报</title>
					...
					timeTicket = setInterval(function (){
						...
						if(i < length) {
						    ...
						    axisData = (new Date()).toLocaleTimeString().replace(/^\D*/,'');
						     ...
						     var name = "+" + resName[i]+"组";
						     var id = resId[i];
						     $.getJSON('st_dongtai_result.php?day=<?php echo $day ?>&dzz_id='+id, function(stdata) {
								   
								totalYd = totalYd + Number(stdata['total_yd']);
								totalFx = totalFx + Number(stdata['total_wz']);		

								var report = document.getElementById("report");
								report.text = "总接触:" +totalYd + " 总分享:"+totalFx;
